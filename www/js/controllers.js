...
function MainCtrl($http) {
    /**
     * Menu Set up - Used as navigation content in navigation view
     */
				...
				'<a uib-dropdown-toggle href>'+
					'<span class="clear">'+
						'<span class="block m-t-xs">'+
							'<strong class="font-bold">'+user_data.USER_NAME+'</strong>'+
						'</span>'+
						'<span class="text-muted text-xs block">'+user_data.STAFF_LEVEL+'</span>'+
					'</span>'+
				'</a>'+
			'</div>'+
			'<div class="logo-element">RA+</div>'+
		'</li>';
		this.appMenu += "<li ng-class='{active: $state.includes('ra')}'><a href='#/dashboards/dashboard_2'><i class='fa fa-home'></i> <span class='nav-label'>Dashboard</span></li>";		
		
		if(window.localStorage.getItem('MENU_DATA') === null) {
			var menu = "";
		}
		else {
			var menu = JSON.parse(window.localStorage.getItem('MENU_DATA'));
		}
		 		
		for(var start = 0; start < menu.length; start++) {
			var inner_menu = JSON.stringify(menu[start]);
			var my_menu = JSON.parse(inner_menu);
			
			
			this.appMenu += "<li ng-class='{active: $state.includes('ra')}'><a href='#'><i class='"+my_menu.menu_name.MODULE_ICON+"'></i>"
			+ "<span class='nav-label'>"+my_menu.menu_name.MODULE_TITLE+"</span>";
			if(my_menu.link_menu.length>0) {
				this.appMenu += "<span class='fa arrow'></span></a>" +
				"<ul class='nav nav-second-level collapse' ng-class='{in: $state.includes('ra')}'>";
				for(var i=0; i< my_menu.link_menu.length; i++) {
					var m = my_menu.link_menu[i];
					if(m.NAV_BAR_VISIBLE==1) {
						var l = m.TASK_URL.replace('/', '.');
						this.appMenu += "<li ui-sref-active='active'><a href='#/"+m.TASK_URL+"' ui-sref='"+l+"'>"+m.TASK_LABEL+"</a></li>";
					}
					else {}
				}
				this.appMenu += "</ul></li>";
...
/*
* Authentication controller to login and logout
*/
function AuthCtrl($scope, $window, $ionicPopup, $state, $http, $ionicLoading) {
	$scope.show = function() {
		$ionicLoading.show({
    ...
    $scope.login = function() {	
		...
		var _headers = {
			...
			'token': '$1$6fjNSBRR$7lx.mxo/q1LbNO7f5.7w8.',
			'UserID':'1'         
		};
		...
		var myCredential = {username: $scope.data.username, password: $scope.data.password};
		console.log(myCredential);
		$http.post('/api/book/adminlogin', JSON.stringify(myCredential), config).then(
		function (response) {
			$scope.result = response;
			if($scope.result.data['status']==200) {			
				console.log($scope.result.data['data']);
				console.log($scope.result.data['menus']);
				window.localStorage.setItem('USER_DATA', JSON.stringify($scope.result.data['data']));	
				window.localStorage.setItem('MENU_DATA', JSON.stringify($scope.result.data['menus']));
				$scope.hide($ionicLoading);
				$state.go('dashboards.dashboard_2', {}, {reload: true});
			}
			else if($scope.result.data['status']!=200) {
				$scope.hide($ionicLoading);
				var alertPopup = $ionicPopup.alert({
					title: 'Login error!',
					template: $scope.result.data['message']
				});
				...
				var alertPopup = $ionicPopup.alert({
					title: 'Login failed!',
					template: 'The cloud server is too busy'
				});
			}
		}, 
		function (response) {
			...
			var alertPopup = $ionicPopup.alert({
				...
				template: 'Please check your connection or close and reopen the application.'
			})
		});
    }
...
function DashCtrl($scope, $http, $ionicPopup) {
	var user_data = JSON.parse(window.localStorage.getItem('USER_DATA'));
	var so = user_data.STAFFID;
	
	$scope.nba_result = {};
	$scope.nba_data = [];
	
	$scope.profit_predition_result = {};
	...
	$scope.lprofit_result = {};
	
	$scope.tprofit_result = {};
	$scope.tprofit_label = [];
	$scope.tprofit_data = [];
	...
	var nba_sqls = "SELECT * FROM VW_NBA WHERE RA_ACT_SO = '"+so+"' AND rownum <= 6 ORDER BY PERCENTAGE DESC";
	var params = {sql:nba_sqls};
	var config = {
		headers : {
			'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8;'
		}
	};
	$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
	function (response) {
		$scope.nba_result = response;
		if($scope.nba_result.status==200) {	
			var nba = $scope.nba_result.data;
			console.log(nba);
			var counter = 1;
			for(var i = 0; i < nba.length; i++) {	
				...
				{
					sn: parseInt(i+1),
					customer: nba[i].RA_ACT_NAME,
					product: nba[i].RA_PRODUCT_NAME,
					percent: (nba[i].PERCENTAGE*100) +'%'
				});
			}
		}
	}, 
	...
	var profit_predition_sql = "SELECT * FROM VW_PROF_PRED WHERE RA_CUST_SO = '"+so+"' AND rownum <= 6";
	var params = {sql:profit_predition_sql};
	var config = {
		headers : {
			'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8;'
		}
	...
	$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
	function (response) {
		$scope.profit_predition_result = response;
		if($scope.profit_predition_result.status==200) {	
			var profit_prediction = $scope.profit_predition_result.data;
			var tmp_d = {};
			tmp_d['x'] = [2011, 2012, 2013, 2014, 2015];
			for(var i = 0; i < profit_prediction.length; i++) {	
				var tmp = [];
				tmp.push(Math.round(parseFloat(profit_prediction[i].YEAR1)));
				tmp.push(Math.round(parseFloat(profit_prediction[i].YEAR2)));
				tmp.push(Math.round(parseFloat(profit_prediction[i].YEAR3)));
				tmp.push(Math.round(parseFloat(profit_prediction[i].YEAR4)));
				tmp.push(Math.round(parseFloat(profit_prediction[i].YEAR5)));
				tmp_d[profit_prediction[i].RA_CUST_NAME] = tmp;
			}
		}
	...
	var chuner_sql = "SELECT * FROM VW_CHURN WHERE RA_ACT_SO = '"+so+"' AND rownum <= 6 ORDER BY PERCENTAGE DESC";
	var params = {sql:chuner_sql};
	var config = {
		headers : {
			'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8;'
		}
	...
	$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
	function (response) {
		$scope.chuner_result = response;
		if($scope.chuner_result.status==200) {	
			var chun_result = $scope.chuner_result.data;
			for(var i = 0; i < chun_result.length; i++) {	
				$scope.chuner_data.push(
				{
					sn: parseInt(i+1),
					customer: chun_result[i].RA_ACT_NAME,
					percent: (Math.round(chun_result[i].PERCENTAGE*100)) +'%'
				});
			}
		}
	}, 
	...
	var clv_sql = "SELECT * FROM VW_CLV WHERE RA_CUST_SO ='"+so+"' AND rownum <= 6";
	var params = {sql:clv_sql};
	var config = {
		headers : {
			'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8;'
		}
	...
	$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
	function (response) {
		$scope.clv_result = response;
		if($scope.clv_result.status==200) {	
			var clv_result = $scope.clv_result.data;
			console.log(clv_result);
			for(var i = 0; i < clv_result.length; i++) {	
				$scope.clv_label.push(
				{
					"id":clv_result[i].RA_CUST_NAME, 
					"type":"donut"
				});
				var tmp = {};
				tmp[clv_result[i].RA_CUST_NAME] = Math.round(parseFloat(clv_result[i].CLV));
				$scope.clv_data.push(tmp);
			}
		}
	}, 
	function (response) {
	...
	var lprofit_sql = "SELECT * FROM (SELECT * FROM vw_profitability WHERE RA_CUST_SO ='"+so+"'"+
	" and to_char(RA_REFERENCE_DATE,'Mon-YYYY')='Jan-1998' ORDER BY RANK_NUM DESC) WHERE ROWNUM <=10";
	var params = {sql:lprofit_sql};
	var config = {
		headers : {
			'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8;'
		}
	...
	$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
	function (response) {
		$scope.lprofit_result = response;
		if($scope.lprofit_result.status==200) {	
			var lprofit_result = $scope.lprofit_result.data;
			console.log(lprofit_result);
			/*
			for(var i = 0; i < predictive.length; i++) {	
				$scope.predictive_data.push(
				{
					pid: parseInt(i+1),
					customer: predictive[i].RA_CUST_NAME,
					description: predictive[i].RECOMMENDATION_DESC,
					action: predictive[i].ACTION_ID,
					recid: predictive[i].RECOMMENDATION_ID
				});
			}*/
		}
	}, 
	...
	var tprofit_sql = "SELECT * FROM vw_profitability WHERE RA_CUST_SO ='"+so+"' and "+
	" to_char(RA_REFERENCE_DATE,'Mon-YYYY')='Jan-1998' AND rownum <= 7";
	var params = {sql:tprofit_sql};
	var config = {
		headers : {
			'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8;'
		}
	...
	$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
	function (response) {
		$scope.tprofit_result = response;
		if($scope.tprofit_result.status==200) {	
			var tprofit_result = $scope.tprofit_result.data;
			console.log(tprofit_result);
			for(var i = 0; i < tprofit_result.length; i++) {	
				$scope.tprofit_label.push("<strong>"+tprofit_result[i].RA_CUST_NAME+"</strong>");
				$scope.tprofit_data.push(tprofit_result[i].PROFITABILITY);
			}
			$scope.tprofit_output = {
				labels: $scope.tprofit_label,
				series: [$scope.tprofit_data]
			};
			console.log($scope.tprofit_label);
			console.log($scope.tprofit_data);
			console.log($scope.tprofit_output);
		}
	}, 
	function (response) {
		var alertPopup = $ionicPopup.alert({
...
// Role Base Access Control Controller
function RbacCtrl() {
	
}
// NBA Recommendation Controller
function NbaRecommendationCtrl($scope, $http, $ionicPopup) {
	...
	var params = {offset:0 , limit:100 , query:'', table:'RECOMMENDATIONS'};
	$http.post('/api/book/', JSON.stringify(params), config).then(
	function (response) {
		$scope.recommend_data = response;
		if($scope.recommend_data.status==200) {			
			var recommendations = $scope.recommend_data.data;
			for(var start = 0; start < recommendations.length; start++) {		
				$scope.recommendations.push(
				{
					id: recommendations[start].RNUM,
					description: recommendations[start].RECOMMENDATION_DESC,
					operation: recommendations[start].RECOMMENDATION_ID
				});
			}
		}
		else if($scope.recommend_data.status!=200) {
			var alertPopup = $ionicPopup.alert({
				title: 'Server error!',
				template: $scope.recommend_data.data['message']
			});	
		}
	}, 
	function (response) {
...
// NBA Recommendation Controller
function NbaPredictiveCtrl($scope, $window, $http, $ionicPopup) {
	$scope.recommendations  = [];
	$scope.predictive_icons = [];
	$scope.predictive_data = [];
	$scope.predictive_data_result = [];
	...
	function (response) {
		...
		if($scope.recommend_data.status==200) {			
			var recommendations = $scope.recommend_data.data;
			for(var start = 0; start < recommendations.length; start++) {
				$scope.predictive_icons.push(
				{
					icon: recommendations[start].RECOMMENDATION_ICON,
					title: recommendations[start].RECOMMENDATION_DESC,
					id: recommendations[start].RECOMMENDATION_ID
				});
				var sqls = 
				"SELECT * FROM NBA LEFT JOIN DIM_CUSTOMER ON DIM_CUSTOMER.RA_CUST_KEY = NBA.CUSTOMER_ID "+
				"LEFT JOIN RECOMMENDATIONS ON RECOMMENDATIONS.RECOMMENDATION_ID = " +
				recommendations[start].RECOMMENDATION_ID+
				" WHERE RECOMMENDATION LIKE '%,"+recommendations[start].RECOMMENDATION_ID+",%'"+
				" OR RECOMMENDATION LIKE '"+recommendations[start].RECOMMENDATION_ID+",%'"+
				" OR RECOMMENDATION LIKE "+"'%,"+recommendations[start].RECOMMENDATION_ID+"'";
				var params = {sql:sqls};
				var config = {
					headers : {
				...
				$http.post('/api/book/freeQuery', JSON.stringify(params), config).then(
				function (response) {
					$scope.predictive_data_result = response;
					if($scope.predictive_data_result.status==200) {	
						var predictive = $scope.predictive_data_result.data;
...
// Contact Controller
function ContactCtrl($scope, $http, $ionicPopup) {
	$scope.contact_data = "";
	$scope.contacts  = [];								
	var config = {
		headers : {
	...
	var params = {offset:0 , limit:100 , query:'', table:'DIM_CUSTOMER'};
	$http.post('/api/book/', JSON.stringify(params), config).then(
	function (response) {
		$scope.contact_data = response;
		if($scope.contact_data.status==200) {			
			var contacts = $scope.contact_data.data;
			for(var start = 0; start < contacts.length; start++) {		
				$scope.contacts.push(
				{
					sn: parseInt(start+1),
					customer: contacts[start].RA_CUST_NAME,
					type: contacts[start].RA_CUST_TYPE,
					branch: contacts[start].RA_CUST_BRANCH,
					gender: contacts[start].RA_CUST_GENDER,
					address: contacts[start].RA_CUST_ADDRESS,
					phone: contacts[start].RA_CUST_TELEPHONE,
					email: contacts[start].RA_CUST_EMAIL
				});
			}
		}
		else if($scope.recommend_data.status!=200) {
...
// All RA Form Hanlder
function FormCtrl($scope, $http, $ionicPopup) {	
	// GET CONTACT DATA
	$scope.contact_data = "";
	$scope.contacts  = [];								
	var config = {
	...
	var params = {offset:0 , limit:100 , query:'', table:'ACTIVITIES'};
	$http.post('/api/book/', JSON.stringify(params), config).then(
	function (response) {
		$scope.activity_data = response;
		if($scope.activity_data.status==200) {			
			var activities = $scope.activity_data.data;
			for(var start = 0; start < activities.length; start++) {		
				$scope.activities.push(
				{
					sn: parseInt(start+1),
					title: activities[start].ACTIVITY,
					id: activities[start].ACTIVITY_ID,
				});
			}
		}
		else if($scope.activity_data.status!=200) {
			var alertPopup = $ionicPopup.alert({
				title: 'Server error!',
				template: $scope.activity_data.data['message']
			});	
		}
	}, 
	function (response) {
	...
	// Activity Model
	$scope.activity = "";
	$scope.activity_add_result = "";
	
	// Add Activity Function
	$scope.addActivity = function() {
		var name = $scope.activity.name;
		var config = {
			headers : {
				'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'
			}
		...
		$http.post('/api/book/create/', JSON.stringify(params), config).then(
		function (response) {
			$scope.activity_add_result = response;
			if($scope.activity_add_result.status==200) {
				$scope.activity.name = "";
				var alertPopup = $ionicPopup.alert({
					title: 'Successful',
					template: $scope.activity_add_result.data['message']
				});
			}
			else if($scope.activity_add_result.status!=200) {
				var alertPopup = $ionicPopup.alert({
					title: 'Server error!',
					template: $scope.activity_add_result.data['message']
				});	
...
// Activity Controller
function ActivityCtrl($scope, $http, $ionicPopup) {
	$scope.activity_data = "";
	$scope.activities  = [];								
	var config = {
		headers : {
...
angular
	...
	.controller('RbacCtrl', RbacCtrl)
	.controller('NbaRecommendationCtrl', NbaRecommendationCtrl)
	.controller('NbaPredictiveCtrl', NbaPredictiveCtrl)
